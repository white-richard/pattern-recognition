...
def mahalanobis_distance(*, x: np.ndarray, mean: np.ndarray, cov: np.ndarray) -> np.ndarray:
    """Mahalanobis distance from each sample to a class mean.

    d(x, µ_i) = sqrt((x - µ_i)^T ∑_i^{-1} (x - µ_i))
    """
    inv_cov = np.linalg.inv(cov)
    diff = x - mean
